...
contract TaskManage is Owned {
    ...
    function initiateTask (string toolName, 
        ...
        task.isCrossValidate = isCrossValidate;
    ...
    function crossValidate()
        internal view returns (uint bestId) {
        require(task.isFinished,
                "Task not finished");
        uint id = 0;
        for (; id<playerPool.length; id++) {
            if (playerPool[id].status == 4) {
                bestId = id++;
                break;
            }
        }
        ...
        for (; id<playerPool.length; id++) {
            if (playerPool[id].status != 4)
                continue;
            if (playerPool[bestId].result > playerPool[id].result)
                bestId = id;
            else if (playerPool[bestId].result == playerPool[id].result &&
                     playerPool[bestId].timeStamp > playerPool[id].timeStamp)
                bestId = id;
        }
    }

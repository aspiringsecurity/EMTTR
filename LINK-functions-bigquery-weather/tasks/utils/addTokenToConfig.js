...
const getOauthToken = async () => {
  const jwt = createJWT(process.env.GOOGLE_ISS, process.env.GOOGLE_KEY);
  ...
  try {
    ...
    if (requestStatus !== 200) {
      throw Error(
        `Error fetching OAuth token: HTTP Status is ${requestStatus}`
      );
    }
...
const addTokenToConfigSecrets = async (requestConfig) =>{
  const authToken = await getOauthToken();
  requestConfig.secrets.authToken = authToken;
  return requestConfig;
}

module.exports = addTokenToConfigSecrets;

...
async function main() {
  ...
  // ID of the prediction market you want to buy NO tokens for.
  const id = 1;
  ...
  try {
    const txn = await MarketHandlerContract.buyNoToken(amount);
    const receipt = await txn.wait(1);
    if (receipt.status == 1) console.log("Bought " + amount/1000000n + " NO tokens");
  } catch (err) {
    console.error(err);
  }
}
